...
>Рекомендуется при настройке виртуального телефон (эмулятора) указывать SDK 36+. Если этого не сделать, то встроенный в Android Studio Linter будет показывать 9+ ошибок, связанных с UsbManager. Это будет мешать разработке приложения.
...
Следует заметить, что изменения прошедшие за три года не такие уж и большие, как можно было ожидать. Если рассматривать конкретно данное приложение, то потребовалось:

- чуть переработать запрос права на использование USB
- чуть изменился механизм передачи управления в другую Activity с использованием Intention
- немного переписать многопоточность (изменилась модель в прикладной библиотеке)

Можно предположить, что для сохранения актуальности кодовой базы достаточно раз в полгода пересобирать приложение и мигрировать на актуальные версии библиотек.

Однако это не касается концептуальных вещей, того как следует разрабатывать современные приложения:

- переход на декларативный Jetpack Compose
- использование новых языковых возможностей Kotlin

# Использование микроконтроллера Raspberry Pi Pico для отладки кода взаимодействия Android-приложения с приборами BVS 

Пример приложения для **CircuitPython**, которая использует библиотеку asyncio для того, чтобы обрабатывать запросы по CDC и мигать LED-ом доступна [здесь](https://github.com/Kerminator1973/RPIDev/blob/main/microcontrollers/pico2040asyncio.md).

Более простой пример приложения для проверки работоспособности Android-приложения (файл "code.py"):
...
Для активации USB CDC, необходимо создать на микроконтроллере файл "boot.py" и добавить в него следующий код:

```py
import usb_cdc

# Включить консоль (REPL) и дополнительный порт для данных
...
## Проверка работоспособности микроконтроллера с помощью приложения на ПК

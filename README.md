...
- создан отдельный файл "Extensions.kt" в который вынесены вспомогательные функции, например, функция toHex(), представляющая массив байт в виде текстового дампа, разбитого на строки по 12 значений

Сначала для работы со списками был применён шаблон **ViewHolder**, который позволяет повторно использовать View, повышая эффективность работы ListView. Однако ListView всё таки является устаревшим решением и он был заменён на **RecyclerView**.

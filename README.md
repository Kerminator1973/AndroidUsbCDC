...
**Задача**: подключить USB-устройство к Android-телефону по OTG. Практическая ценность задачи - в России существует огромный парк счётчиков и валидаторов купюр. Каждый раз, когда ЦБ РФ выпускает новый номинал, или когда в прошивке устройства найдена ошибка, возникает потребность в обновлении прошивки прибора. Сейчас сервис-инженер может выполнить эту задачу с помощью ноутбука, но кажется избыточным таскать с собой тяжёлый ноутбук для обновления прошивок приборов. Мобильный телефон имеет доступ в интернет и прибор для обновления можно подключить через USB-порт.

Данный репозитарий позволяет проверить, можно ли надёжно выполнять обновление приборов через мобильный телефон, с использованием USB-подключения.

Пользовательский интерфейс приложения может выглядеть следующим образом:

![screen shot](./main-screen.png)

Для имитации счётчика используется микроконтроллер Raspberry Pi Pico, для которого разработана прошивка, имитирующая некоторую часть протокола взаимодействия с реальным счётчиком купюр.

## Стартовые требования и стенд разработки
...
На некоторых телефонах, например, на Realme 9 Pro 5G технология OTG "по умолчанию" отключена. Для того, чтобы активировать её необходимо перейти в "Настройках" в раздел "Additional Settings" и установить "OTG connection" в значение "On".
...
Tools -> Device Manager выбрать закладку "Phisical" и настроить отладку по Wi-Fi. Телефон предварительно нужно настроить для удалённой отладки, что можно сделать в Developer Options -> Wireless Debugging. Если нажать на пункт "Wireless Debugging", то можно перейти на форму, на которой доступен pairing устройства и Android Studio. Выполнить pairing можно либо по QR-коду (на Realme 9 Pro 5G не сработало), либо по PIN-коду (успешно запустилось). Отладка по Wi-Fi вполне работоспособна.

Следует заметить, что отладка работает только на устройствах с установленным Android 11+. 

>Смартфон Realme 9 Pro 5G на котором установлен Android 14 для этой задачи подходит. И отладка, и OTG-работоспособны. Однако следует иметь ввиду, что телефон может автоматически отключать OTG через 10 минут, что чаще всего приводит к тому, что при попытке получить доступ к драйверу USB CDC, приложение получает пустой список доступных драйверов.
...
Для отладки кода Android-приложения, рекомендуется использовать микроконтроллер Raspberry Pi Pico. Инструкция по подготовке прошивки микроконтроллера к работе с Android-приложением приведена [здесь](./cdc-exchange.md).
...
Прошивка эмулирующая валидатор и счётчик ДОРС [описана здесь](./cdc-exchange.md). Там же находятся исходник кода на Python для ПК, который проверяет работоспособность прошивки Raspberry Pico.
...
>Уточненный вариант см. в файле "code-migration.md"
...
Класс **SerialInputOutputManager** реализует интерфейс **Runnable** и предоставляет интерфейс **Listener**, у которого есть два callback-метода: onNewData и onRunError. Когда вызывается статический метод newSingleThreadExecutor, он создаёт ThreadPoolExecutor содержащий только один поток исполнения и запускает в этом потоке Runnable Task - SerialInputOutputManager, который периодически пытается получить данные из последовательного порта. Поскольку мы устанавливаем бесконечный тайм-аут, эта задача будет выполняться бесконечно. Т.е. если мы попытаемся открыть ещё один порт (на Raspberry Pi Pico их два), то запуститься ещё один поток исполнения и каждый из них будет передавать callback-функцию onNewData данные, по мере их поступления.

Метод **close**() класса **UsbSerialPort** изменяет состояние задачи mState на STOPPING, что приведёт к остановке задачи через некоторое время:
...
После выполненной выше операции, мы уже не сможем получить информацию о driver.ports[1], если только не вызовем `manager.openDevice(driver.device)` ещё раз.
...
Следует заметить, что оба разработчика (и польский, и японец) использовали "device_filter.xml". Например, японский разработчик подключал только Raspberry Pi Pico:
...
## Структурный рефакторинг приложения (июнь 2026 года)

В рамках задачи по структурному рефакторингу были выполнены структурные изменения, снижающие сцепленность кода:

- создан отдельный класс **AppPreferences** в котором осуществляется вся работа с Preferences. Подход позволяет избежать использования "магических" значений и дублирования кода
- создан отдельный файл "Extensions.kt" в который вынесена вспомогательная функция функция toHex(), представляющая массив байт в виде текстового дампа, разбитого на строки по 12 значений
- список портов, который был реализован как ListView, без применения шаблона **ViewHolder**, был заменён на **RecyclerView**. При этом, для отправки в MainActivity сообщения о выборе коммуникационного порта используется не дополнительный интерфейс, а лямбда-функция, чтобы не засорять _global scope_
- взаимодействие между MainActivity и UsbConnectionManager осуществляется посредством Coroutines/Flow
...
Ожидаемая диаграмма классов:

![Диаграмма классов](./classes-diagram.png)

...
Название компонента **RecyclerView** точно отражает его суть - это отображение (View), т.е. композиционная схема, которая повторно использует ресурсы, чтобы добиться лучшей утилизации вычислительных ресурсов и большей производительности. "Под капотом" RecyclerView использует ту же концепцию ViewHolder, но избавляют программиста от ручного создания ссылок на View. Также при использовании RecyclerView исчезает потребность в наследовании от BaseAdapter.

Ознакомиться с общим описанием RecyclerView можно на сайте [Яндекс.Практикум](https://practicum.yandex.ru/blog/kak-rabotat-s-recyclerview/).
